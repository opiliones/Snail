...
dictToList d = concat $ map dictToList' $ H.toList d
  where
...
del env xs@(_:_) =
  case last xs of
    ...
    List l -> do
      ...
      case ys of
        Just ns -> delList env l (reverse $ sort ns)
        _ -> usagePrint "del"
    _ -> usagePrint "del"
  where
